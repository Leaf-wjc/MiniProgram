var articleData = [
  {
    author_img: '/images/author/author3.jpeg',
    author_date: 'Sep 18 2020',
    article_tit: '唯愿山河无恙·再无国殇。',
    article_img: '/images/article/article3.jpeg',
    article_content: '又逢芳草绿，清明祭英雄，带着哀思我们缅怀在这场疫情中逝去的他们，谁家女非掌中玉，谁家儿不负家冀。疫情肆虐无情时，仍有人举火抱薪,为守得山河无恙，岁月长安.医者、警察、教师、基层工作者、志愿者英雄们筑起守护长城。\n深切缅怀！',
    collect_num: '999+',
    reading_num: '999+',
    headImgSrc:'/images/article/article3.jpeg',
    author_name:'Author1',
    dateTime:'24小时前',
    detail:'菊黄蟹正肥，品尝秋之味。徐志摩把“看初花的荻芦”和“到楼外楼吃蟹”并列为秋天来杭州不能错过的风雅之事；用林妹妹的话讲是“螯封嫩玉双双满，壳凸红脂块块香”；在《世说新语》里，晋毕卓更是感叹“右手持酒杯，左手持蟹螯，拍浮酒船中，便足了一生矣。”漫漫人生长路，美食与爱岂可辜负？于是作为一个吃货，突然也很想回味一下属于我的味蕾记忆。记忆中的秋蟹，是家人的味道，弥漫着浓浓的亲情。\n\n是谁来自山川湖海，却囿于昼夜，厨房与爱？ 是母亲，深思熟虑，聪明耐心。吃蟹前，总会拿出几件工具，煞有介事而乐此不疲。告诉我们螃蟹至寒，需要佐以姜茶以祛寒，在配备的米醋小碟里，亦添入姜丝与紫苏，前者驱寒后者增香。泡好菊花茶，岁月静好，我们静等。',
    newsId: 0
  },
  {
    author_img: '/images/author/author1.jpg',
    author_date: 'Sep 18 2016',
    article_tit: '千与千寻:记忆重庆·没有森林',
    article_img: '/images/article/article1.jpg',
    article_content: '虽然锦城与山城一家亲,第一次完整的拥入她还是在两年前的初夏；网络上重庆的魔幻名气伊始，千与千寻的画面感让越来越多的人心驰神往，除了这些，最好体验给我的还有满城的烟火气，路边摊的小面与九宫格的火锅香~~~',
    collect_num: '42',
    reading_num: '196',
    headImgSrc:'/images/article/article1.jpg',
    author_name:'Author2',
    dateTime:'24小时前',
    detail:'动车下车后，坐上轻轨就先去酒店安顿，观看过那么多的轻轨穿梭视频，第一次切实的体验是极好的，群山穿越，江边蜿蜒，两岸猿声啼不住，“轻轨”已穿万重山；地形复杂的山中城市也能够亭台楼阁，层峦叠嶂，十步一景，百步一新。\n作为一个爱吃面食的四川人，相比于北方人喜欢面本身的劲道麦香，川渝地区大多数吃的是它的料；重庆小面就是具有代表的吃面吃料，味道好不好，重在调料，每一家客人络绎不绝的面馆，他的味道必然过硬，也有自己的独家调料水准。',
    newsId: 1
  },
  {
    author_img: '/images/author/author2.jpg',
    author_date: 'Sep 04 2020',
    article_tit: 'Switch:给你更多可能',
    article_img: '/images/article/article2.jpg',
    article_content: 'Joy-Con是Switch主机的游戏手柄，它是可拆分的。拆分后，由原来的掌上模式变成电视模式或桌面模式。一机三用，可玩性相当高。早就2006年Wii发布的时候，就已经有体感手柄，但是长期玩体感很容易疲劳；到了2011年改善版WiiU登场，采用了两块屏幕，掌机的屏幕又得不到充分利用~~~',
    collect_num: '46',
    reading_num: '999',
    headImgSrc:'/images/article/article2.jpg',
    author_name:'Author3',
    dateTime:'24小时前',
    detail:'Joy-Con是Switch主机的游戏手柄，它是可拆分的。拆分后，由原来的掌上模式变成电视模式或桌面模式。一机三用，可玩性相当高。早就2006年Wii发布的时候，就已经有体感手柄，但是长期玩体感很容易疲劳；到了2011年改善版WiiU登场，采用了两块屏幕，掌机的屏幕又得不到充分利用~~~,Joy-Con是Switch主机的游戏手柄，它是可拆分的。拆分后，由原来的掌上模式变成电视模式或桌面模式。一机三用，可玩性相当高。早就2006年Wii发布的时候，就已经有体感手柄，但是长期玩体感很容易疲劳；到了2011年改善版WiiU登场，采用了两块屏幕，掌机的屏幕又得不到充分利用~~~,Joy-Con是Switch主机的游戏手柄，它是可拆分的。拆分后，由原来的掌上模式变成电视模式或桌面模式。一机三用，可玩性相当高。早就2006年Wii发布的时候，就已经有体感手柄，但是长期玩体感很容易疲劳；到了2011年改善版WiiU登场，采用了两块屏幕，掌机的屏幕又得不到充分利用~~~',
    newsId: 2
  }
]

module.exports = {
  homeList: articleData
}